...
public class DailyScheduler implements SchedulingConfigurer {
	...
	public void dailyOverAllRunnerMorning() {
		...
		Mono<String> response = webClient.get().uri("/portal/scheduler/DailyOverAllRunner", 10L).accept(MediaType.TEXT_HTML)
				.retrieve().bodyToMono(String.class);
		logger.info("dailyOverAllRunnerMorning stop, response: {}", response.toString());
	}
	...
	public void dailyOverAllRunnerAndSanityAfternoon() {
		...
		Mono<String> response = webClient.get().uri("/portal/scheduler/DailyOverAllRunnerAndSanity", 10L).accept(MediaType.TEXT_HTML)
				.retrieve().bodyToMono(String.class);
		logger.info("dailyOverAllRunnerAndSanityAfternoon stop, response: {}", response.toString());
	}
	...
	public void dataBaseSanityCheck() {
		...
		Mono<String> response = webClient.get().uri("/portal/scheduler/DataBaseSanityCheck", 10L).accept(MediaType.TEXT_HTML)
				.retrieve().bodyToMono(String.class);
		logger.info("dataBaseSanityCheck stop, response: {}", response.toString());
	}
	...
	public void dailyUpdateStockPriceByBatch() {
		...
		Mono<String> response = webClient.get().uri("/portal/scheduler/DailyUpdateStockPriceByBatch", 10L).accept(MediaType.TEXT_HTML)
				.retrieve().bodyToMono(String.class);
		logger.info("dailyUpdateStockPriceByBatch stop, response: {}", response.toString());
	}
}
