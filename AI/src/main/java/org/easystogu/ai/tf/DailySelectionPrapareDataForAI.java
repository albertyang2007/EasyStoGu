...
public class DailySelectionPrapareDataForAI {
	...
	private void prepareCsvData(String stockId) {
		...
		for (String date : dates) {
			List<CheckPointDailySelectionVO> cpDailySelections = checkPointDailySelectionTableHelper
					.getCheckPointSelection(stockId, date);
	...
	// 稀疏矩阵
	private String[] toCheckPointDailySelectionArray(List<CheckPointDailySelectionVO> cpDailySelectionVos) {
	...
	private String[] toPricePredictResultArray(String stockId, String date) {
		...
		// high price
		double highPrice = qianFuQuanStockPriceTableHelper.getHighPriceStartDate(stockId, date,
				predictPriceWithinNextNDays);
		if (highPrice == 0.0) {
			return array;
		}
		...
		// low price
		double lowPrice = qianFuQuanStockPriceTableHelper.getLowPriceStartDate(stockId, date,
				predictPriceWithinNextNDays);
		if (lowPrice == 0.0) {
			return array;
		}
