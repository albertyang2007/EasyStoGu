...
public class StockPriceTableHelper implements CacheAbleStock {
	...
	protected String SELECT_HIGH_N_PRICE_START_DATA_SQL = "SELECT max(high) AS rtn from (SELECT high FROM " + tableName
			+ " WHERE stockId = :stockId AND date > :startDate LIMIT :limit) AS myhighn";
	// AI use this Low(n) with date start from
	protected String SELECT_LOW_N_PRICE_START_DATA_SQL = "SELECT min(low) AS rtn from (SELECT low FROM " + tableName
	...
	protected String COUNT_BY_DATE_SQL = "SELECT COUNT(*) AS rtn FROM " + tableName + " WHERE DATE = :date";
	...
	public List<StockPriceVO> getStockPriceByIdAndBetweenDate(String stockId, String StartDate, String endDate) {
		try {
			MapSqlParameterSource namedParameters = new MapSqlParameterSource();
			namedParameters.addValue("stockId", stockId);
			namedParameters.addValue("date1", StartDate);
