...
public class TALIBWraper {
	...
	public double[][] getMACD(double[] prices, int optInFastPeriod, int optInSlowPeriod, int optInSignalPeriod) {
		/*
		 * optInFastPeriod(����)��optInSlowPeriod(����)��optInSignalPeriod(�ź���
		 * �� �ţͣ�n=��n*2/��n+1��+�ţͣ�n-1*��n-1��/��n+1�� ʽ��
		 * ��n:�������̼�;�ƶ�ƽ�������ڣţͣ�n:��n�գţͣ�ֵ
		 * ��һ������£����٣ţͣ�һ��ѡ���գ����٣ţͣ�һ��ѡ12�գ���ʱ����ֵ���ģɣƣ��ļ���Ϊ��
	...
	public double[] getCci(double[] highPrices, double[] lowPrices, double[] closePrices, int inTimePeriod) {
		/*
		 * (1) TP=����߼�+��ͼ�+���̼ۣ���3 �� (2) SMA TP=SMA (TP, N)ע�⣺NΪ�������ڡ�
		 * ��3��MD��Mean Deviation��Ҳ����ƽ����ֵ MD=�Ʀ�TP-SMA TP���Nע�⣺�ƴ��Ϊ�ܺ�
		 * ��4�������CCI��ֵ ��CCI=��TP-SMA TP����(0.015��MD)
		 */
		double[] output = new double[lowPrices.length];
		double[] tempOutPut = new double[lowPrices.length];
	...
	public double[][] getMacdExt(double[] prices, int optInFastPeriod, int optInSlowPeriod, int optInSignalPeriod) {
		 ...
		 * DEA�ߡ���Difference Exponential Average��DIFF�ߵ�M��ָ��ƽ���ƶ�ƽ����
		 ...
		 * ��ָ��ƽ����ָ������ָ��ʽ�ݼ���Ȩ���ƶ�ƽ��
		 * ����ֵ�ļ�Ȩ����ʱ���ָ��ʽ�ݼ���Խ���ڵ���ݼ�ȨԽ�أ����Ͼɵ����Ҳ����һ���ļ�Ȩ��
		 * �����ʣ��ģɣƣ�=12��EMA-26��EMA����DIFƽ��ֵ��DEA��=���9�յ�DIF֮��/9
		 * �ͣ��ã�=�����յ�DIFF-���յ�DEA����2
		 */
		double[] tempoutput1 = new double[prices.length];
		double[] tempoutput2 = new double[prices.length];
	...
	public double[] getObv(double[] prices, double[] volume) {
		/*
		 * ����OBV=����OBV+sgn������ĳɽ�������sgn�Ƿ�ŵ���˼��sgn������+1��Ҳ������-1�
		 * �������ʽ������ Sgn=+1 �����̼ۡ������̼�Sgn=�D1
		 * �����̼�<�����̼۳ɽ���ָ���ǳɽ���Ʊ�������ǳɽ���
		 */
		double[] output = new double[prices.length];
	...
	public double[][] getStochF(double[] highPrices, double[] lowPrices, double[] closePrices, int optInFastK_Period,
		...
		/*
		 * optInFastK_Period:��K���ڣ�optInSlowK_Period����K���ڣ�optInSlowD_Period
		 * ����D���� STOCH=Stochastic�����ָ��,KDJ��K %K = 100*
		 * (LOSE-LOW(%K))/(HIGH(%K)-LOW(%K)) CLOSE �� ��������̼۸�LOW(%K) ��
		 * %K�����ֵ��HIGH(%K) �� %K�����ֵ %D���ƶ�ƽ���ߣ�%D = SMA(%K�� N)
		 */
	...
	public double[][] getStoch(double[] highPrices, double[] lowPrices, double[] closePrices, int optInFastK_Period,
		 ...
		 * ����D���� STOCH=Stochastic�����ָ��,KDJ��K %K = 100*
		 * (LOSE-LOW(%K))/(HIGH(%K)-LOW(%K)) CLOSE �� ��������̼۸�LOW(%K) ��
		 * %K�����ֵ��HIGH(%K) �� %K�����ֵ MaxHigh(N) - N ������ǰ����ߵ㣻 MinLow(N)
		 * - N ������ǰ����͵㣻 MA - �ƶ�ƽ���ߣ�N - ��/�ͷ�Χ�ļ��㳤�ȣ� P -
		 * %D(i)���˲����ڡ�
		 */
	...
	public double[] getTrix(double[] prices, int period) {
		/*
		 * (1)����N������̼۵�ָ��ƽ��AX AX = (I��) ���̼� * 2 /(N+1) + (I-1)�� AX
		 * (N-1) *(N+1) (2)����N���AX��ָ��ƽ��BX BX = (I��) AX * 2 /(N+1) +
		 * (I-1)�� BX (N-1) *(N+1) ��3)����N���BX��ָ��ƽ��TRIX TRIX = (I��) BX *
		 * 2 /(N+1) + (I-1)�� TRIX (N-1) *(N+1) (4)����TRIX��m���ƶ�ƽ��TMA TMA =
		 * ((I-M)��TRIX�ۼ�) /M��
		 */
		double[] output = new double[prices.length];
		double[] tempOutPut = new double[prices.length];
	...
	public double[] getWillR(double[] highPrices, double[] lowPrices, double[] closePrices, int inTimePeriod) {
		/*
		 * n��WMS����Hn��Ct��/(Hn��Ln)��100��CtΪ��������̼ۣ�Hn��Ln�����n���ڣ������
		 * ��죩���ֵ���߼ۺ���ͼ�
		 */
		double[] output = new double[lowPrices.length];
		double[] tempOutPut = new double[lowPrices.length];
	...
	public double[] getAd(double[] highPrices, double[] lowPrices, double[] closePrices, double[] inVolume,
			int optInTimePeriod) {
		/*
		 * AD=Chaikin A/D Line���ۻ��ɷ���ָ�� CLV���㹫ʽ�� {[(C-L)-(H-C)]}/(H-L)) =
		 * CLV C�����̼ۡ�L�����յ͵�H�������ոߵ㡣
	...
	public double[] getAdosc(double[] highPrices, double[] lowPrices, double[] closePrices, double[] inVolume,
		...
		/*
		 * (1)ADOSC=Chaikin A/D Oscillator������ָ�� (2) CHAIKIN��A/D�ģ�n��expma
		 * - A/D�ģ�m��expma��
		 */
		double[] output = new double[lowPrices.length];
		// double[] tempOutPut = new double[lowPrices.length];
	...
	public double[] getAroonOsc(double[] inHigh, double[] inLow, int optInTimePeriod) {
		/*
		 * AROONOSC=Aroon Oscillator����¡��ָ�� ͨ����㰢¡�����(Aroon up and
		 * down)֮��ֵ����
		 */
	...
	public double[] getCmo(double[] closePrices, int period) {
		/*
		 * CMO=Chande Momentum Oscillator��Ǯ�¶����ڶ�ָ�ꡣ CMO =��Su-Sd) * 100 /
		 * (Su +Sd)
		 * u�ǽ������̼����������̼ۣ������գ���ֵ���ܡ��������µ�������ֵΪ0��Sd�
		 * ǽ������̼����������̼� ���µ��գ���ֵ�ľ��ֵ���ܡ� ���������ǣ�������ֵΪ0
		 */
	...
	public double[] getTrima(double[] prices, int optInTimePeriod) {
		/*
		 * TRIMA=Triangular Moving Average���ݹ��ƶ�ƽ��ָ�� SMA = (P1 + P2 + P3 +
