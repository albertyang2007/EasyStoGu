...
public class CombineAnalyseHelper {
	...
	public boolean isConditionSatisfy(DailyCombineCheckPoint checkPoint, List<StockSuperVO> overDayList,
		...
		case TiXing_SuoLiang_LaSheng_Close_Price_Higher_Volume_Lower: {
			// example:
			// close price is higher and higher
			if ((curSuperDayVO.priceVO.close > pre1SuperDayVO.priceVO.close)
					&& (pre1SuperDayVO.priceVO.close > pre2SuperDayVO.priceVO.close)
					&& (pre2SuperDayVO.priceVO.close > pre3SuperDayVO.priceVO.close)) {
				// but volume is lower and lower
				if ((curSuperDayVO.priceVO.volume < pre1SuperDayVO.priceVO.volume)
						&& (pre1SuperDayVO.priceVO.volume < pre2SuperDayVO.priceVO.volume)
						&& (pre2SuperDayVO.priceVO.volume < pre3SuperDayVO.priceVO.volume)) {
					// skip the wuLiang zhangTing
					if (curSuperDayVO.priceVO.isKLineRed(1.0, 9.9) && pre1SuperDayVO.priceVO.isKLineRed(1.0, 9.9)
							&& pre2SuperDayVO.priceVO.isKLineRed(1.0, 9.9)
							&& pre3SuperDayVO.priceVO.isKLineRed(1.0, 9.9)) {
						return true;
					}
				}
			}
		...
		case DuoTou_Close_And_MA5_Higher_MA120: {
			// example: 600644 20150310
			// volume and price is the highest in half year
			if ((curSuperWeekVO.kdjVO.k < curSuperWeekVO.kdjVO.d) || !this.isLatestKDJCrossGordon(overWeekList)) {
				// over all week KDJ must after Gordon
				return false;
			...
			if (curSuperDayVO.avgMA5 >= curSuperDayVO.avgMA10 && curSuperDayVO.avgMA10 >= curSuperDayVO.avgMA20
				...
				if (curSuperDayVO.avgMA10 >= pre1SuperDayVO.avgMA10 && curSuperDayVO.avgMA20 >= pre1SuperDayVO.avgMA20
					...
					if (pre1SuperDayVO.priceVO.close < pre1SuperDayVO.avgMA120
						...
						// now close and ma5 > ma120
						if (curSuperDayVO.priceVO.close > curSuperDayVO.avgMA120
								&& curSuperDayVO.avgMA5 > curSuperDayVO.avgMA120) {
							return true;
						}
					}
				}
	...
	private boolean MA5_MA10_Ronghe_Close_Higher(StockSuperVO curSuperDayVO, StockSuperVO pre1SuperDayVO) {
		// rongHe and xiangShang
		double dif = Math.abs(curSuperDayVO.avgMA5 - curSuperDayVO.avgMA10);
		double min = Math.min(curSuperDayVO.avgMA5, curSuperDayVO.avgMA10);
		// MA rongHe
