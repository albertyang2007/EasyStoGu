...
public class PostgreSqlDataSourceFactory {
	private static Logger logger = LogHelper
			.getLogger(PostgreSqlDataSourceFactory.class);
	private static FileConfigurationService config = FileConfigurationService
			.getInstance();
	...
	public static javax.sql.DataSource createDataSource() {
		...
		int active = config.getInt(Constants.JdbcMaxActive, 1);
		int idle = config.getInt(Constants.JdbcMaxIdle, 1);
