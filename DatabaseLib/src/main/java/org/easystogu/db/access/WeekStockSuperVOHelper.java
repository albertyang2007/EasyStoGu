...
public class WeekStockSuperVOHelper extends StockSuperVOHelper {
    ...
    public List<StockSuperVO> getLatestNStockSuperVO(String stockId, int day) {
        ...
        if ((spList.size() != day) || (macdList.size() != day) || (kdjList.size() != day)
                || (mai1mai2List.size() != day) || (yiMengBSList.size() != day)) {
            return overList;
        }
    ...
    public List<StockSuperVO> getAllStockSuperVO(String stockId) {
        ...
        /*
         * if ((spList.size() != macdList.size()) || (macdList.size()
         * != kdjList.size()) || (kdjList.size() != spList.size()) ||
         * (mai1mai2List.size() != spList.size())) { //
         * System.out.println(stockId + " size of spList(" +
         * spList.size() + // "), macdList(" + macdList.size() // +
         * ") and kdjList(" + kdjList.size() + //
         * ") is not equal, the database must meet fatel error!");
         * return overList; }
         * 
         * if ((spList.size() == 0) || (macdList.size() == 0) ||
         * (kdjList.size() == 0) || (mai1mai2List.size() == 0)) {
         * return overList; }
         * 
         ...
         * 
         * for (int index = 0; index < spList.size(); index++) {
         * StockSuperVO superVO = new StockSuperVO(spList.get(index),
         * macdList.get(index), kdjList.get(index), null);
         * superVO.setMai1Mai2VO(mai1mai2List.get(index));
         * overList.add(superVO); }
         */
        return overList;
